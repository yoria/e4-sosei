...
def sort_json_by_similarity(json_path, key):
    json_open = open(json_path, 'r')
    json_load = json.load(json_open)
    return sorted(json_load, key=lambda x: x[key])
...
def get_accuracy(json_load):
    ...
    for c in json_load:
        for sc in seikai_c:
            ...
            #print(sc[0])
            #print(sc[1])
            if {sc[0], sc[1]} == {convert_path(c['s1']), convert_path(c['s2'])}:
                seikai_num += 1
                break;
